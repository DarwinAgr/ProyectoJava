...
import Esfe.Persistencia.UsuarioDAO; // Importa la clase UserDAO desde el paquete esfe.persistencia. Esta clase se encarga de la interacción con la base de datos para la entidad User (crear, leer, actualizar, eliminar, autenticar usuarios).
...
public class LoginForm extends JDialog {
    ...
    private void login() {
        try{
            ...
            Usuario userAut = userDAO.authenticate(user); // Llama al método 'authenticate' del objeto 'userDAO' para verificar las credenciales del usuario contra la base de datos. El resultado (un objeto User si la autenticación es exitosa, o null si falla) se almacena en 'userAut'.
            ...
            if(userAut != null && userAut.getIdUsuario() > 0 && userAut.getEmail().equals((user.getEmail()))){
